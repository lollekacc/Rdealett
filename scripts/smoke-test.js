...
const runSmokeTests = async ({ baseUrl, debugBase }) => {
  ...
  await mobile.waitFor(`document.querySelectorAll('.plan-card').length > 0`, 10000);
  await mobile.waitFor(`document.querySelector('#dataFilter')?.dataset.values?.includes('10')`);
  await mobile.evaluate(`(() => {
    const slider = document.querySelector('#dataFilter');
    slider.value = slider.dataset.values.split(',').indexOf('10');
  ...
  await mobile.evaluate(`document.querySelector('#dataFilterAll').click()`);
  ...
  await family.waitFor(`document.querySelectorAll('.plan-card').length > 0`, 10000);
  await family.evaluate(`document.querySelector('#dataFilterAll').click()`);
  await family.waitFor(`document.querySelectorAll('.plan-card').length >= 10`, 10000);
  await family.evaluate(`(() => {
    const slider = document.querySelector('#familySize');
    slider.value = '4';
    slider.dispatchEvent(new Event('input', { bubbles: true }));
    slider.dispatchEvent(new Event('change', { bubbles: true }));
  })()`);
