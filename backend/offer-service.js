...
const buildBroadbandCartItem = ({ planId }) => {
  ...
  const reward = calculateBroadbandReward(plan.price);
  const logo = providerLogos[plan.operator] || '';
  const cartItem = {
    cartItemId: `${plan.id}-${Date.now()}`,
    offerId: plan.id,
    operator: plan.operator,
    title: plan.title || plan.speed || '5G-bredband',
    logo,
    data: plan.speed,
    price: plan.price || 0,
    pricePerPerson: 0,
    ...
    features: [
      formatBinding(plan),
      `${String(plan.technology || '').toUpperCase()} · ${formatCurrency(plan.speedMbps)} Mbit/s`,
      ...(plan.features || []),
    ].filter(Boolean),
  };
